...
import { validateFreshAccessoryPayload } from '../freshness.js';
...
export async function handleAccessories(args, send = sendCommand) {
...
export async function handleAutomations(args) {
  ...
  switch (action) {
    ...
    case 'add_condition': {
      ...
      // 'characteristic' is a documented alias for 'property' (issue #76) —
      // normalize here so the socket layer sees the canonical key.
